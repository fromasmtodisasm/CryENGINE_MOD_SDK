...
void getBBoxSSE (const Matrix44* pBones, unsigned numBones, CryBBox* pBBox)
{
	assert (numBones > 0);
	_asm
	{
...
// of their positions, and puts them into the min and max Vec3d
// NOTE: the matrix array must be aligned on 16-byte boundary
void getBBoxSSE (const Matrix44* pBones, const CryBBoxA16* pBoneBBox, unsigned numBones, CryBBox* pBBox)
{
	assert (numBones > 0);
	_asm
	{
